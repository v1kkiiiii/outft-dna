...
Any change to prompt text, taxonomy lists embedded in it, output shape, or tone rules produces a new `promptVersion` (e.g. `outft-analysis-v1` → `outft-analysis-v2` → `outft-analysis-v3`) and must pass the full evaluation gate (Section 6) before deployment. Current: `outft-analysis-v3`. `promptVersion` is recorded on every analysis and every job (PRD §12.1, §17.1).
